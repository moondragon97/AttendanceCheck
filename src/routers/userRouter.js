...
rootRouter.route("/edit-profile").all(protectNotUser).get(getProfileEdit).post(uploadFiles.single('avatar'), postProfileEdit); 
rootRouter.route("/edit-password").all(protectNotUser, protectSocialUser).get(getPasswordEdit).post(postPasswordEdit);
